...
for(current_surface in surfaces$surface){
	...
	y_hat <- predict(model_fit_current, x_train_for_test_ova_current, type = "raw")
	y_hat_prob <- predict(model_fit_current, x_train_for_test_ova_current, type = "prob")
	y_hat_new <- as.factor(ifelse(y_hat_prob[,current_surface] >= 0.35, current_surface, "the_rest"))
	...
	# results <- results %>% mutate(last_result_prob = y_hat_prob[,"the_rest"])
	# names(results)[ncol(results)] <- paste(current_surface, "_the_rest_prob", sep = "")
	...
	# this is thee confusion matrix with a cutoff probability of 35% instead on 50% to account for prevalence
...
write_csv(submission, "data/submission_03.csv")
```

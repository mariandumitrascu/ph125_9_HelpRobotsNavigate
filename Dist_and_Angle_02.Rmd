...
fit_model_RF_untuned <- randomForest(surface ~ ., data = select(x_train_for_train, -series_id))
...
results <- data.frame(series_id = x_train_for_test_ova$series_id, true_surface = x_train_for_test_ova$surface)
...
for(current_surface in surfaces$surface)
{
		...
		# tunegrid <- expand.grid(.mtry=mtry,.ntree=c(500, 1000, 1500, 2000, 2500) )
		tunegrid <- expand.grid(.mtry=mtry,.ntree=c(100, 300, 400, 500, 1000))
		# metric <- "ROC"
		control <- trainControl(method="repeatedcv", number=10, repeats=3, search="grid", classProbs = TRUE, summaryFunction = twoClassSummary)
		
		fit_model_current <- train(surface ~ ., data = x_train_for_train_ova_current, method=customRF, metric="ROC", tuneGrid=tunegrid, trControl=control)
		
		# fit_model_current <- train(surface ~ ., data = x_train_for_train_ova_current,
		# 		method = "rf",
		# 		metric = "ROC",
		...
		y_hat <- predict(fit_model_current, x_train_for_test_ova_current, type = "raw")
		y_hat_prob <- predict(fit_model_current, x_train_for_test_ova_current, type = "prob")
		...
		results <- results %>% mutate(last_result = y_test)
		names(results)[ncol(results)] <- paste(current_surface, "_y_test", sep="")
		
		results <- results %>% mutate(last_result = y_hat)
		names(results)[ncol(results)] <- paste(current_surface, "_y_hat", sep="")

		results <- results %>% mutate(last_result = y_hat_prob[, 1])
		names(results)[ncol(results)] <- paste(current_surface, "_y_hat_prob", sep="")
...
results %>% filter(true_surface == "hard_tiles")
results %>% filter(hard_tiles_y_hat_prob >= 0.30)

```


## Load Models and Perform thee Prediction
```{r}
...
results_prob <- data.frame(series_id = x_train_for_test_ova$series_id, true_surface = x_train_for_test_ova$surface)
results_prob <- results_prob %>% mutate(rowNum = row_number())
...
for(current_surface in surfaces$surface){
	...
	model_name <- paste("fit_model_rf_", current_surface, sep = "")
	fit_model_current <- readRDS(paste("models/", model_name, ".rds", sep = ""))
	
	# get y_hat and y_hat_prob
	y_hat <- predict(fit_model_current, x_train_for_test_ova_current, type = "raw")
	y_hat_prob <- predict(fit_model_current, x_train_for_test_ova_current, type = "prob")
	y_hat_new <- as.factor(ifelse(y_hat_prob[,1] >= 0.3, current_surface, "the_rest"))
	
	# get the y_test
	y_test <-  x_train_for_test_ova_current$surface
	
	# store results
	results <- results %>% mutate(true_surface_current = y_test) 
	
	results <- results %>% mutate(last_result = y_hat)
	names(results)[ncol(results)] <- current_surface
	
	results <- results %>% mutate(last_result_prob = y_hat_prob[,1])
	names(results)[ncol(results)] <- paste(current_surface, "_prob", sep = "")
	
	results <- results %>% mutate(last_result_new = y_hat_new)
	names(results)[ncol(results)] <- paste(current_surface, "_new", sep = "")
	...
	conf_matrix_new <- confusionMatrix(y_hat_new, x_train_for_test_ova_current$surface)
	print(conf_matrix_new$overall["Accuracy"])
	print(conf_matrix_new$table)

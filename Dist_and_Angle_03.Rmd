...
# used for models 8
# ########################################################################################################
# x_train_processed <- x_train_processed %>% mutate(f1 = atan(dist_mean_to_sd/omega_mean_to_sd),
# 																									f2 = atan(theta_mean_all/psi_mean_all))
# x_test_processed <- x_test_processed %>% mutate(f1 = atan(dist_mean_to_sd/omega_mean_to_sd),
# 																									f2 = atan(theta_mean_all/psi_mean_all))
# 
# x_train_processed <- x_train_processed %>% select(series_id, surface,
# 																									phi_mean_all, theta_mean_all,
# 																									phi_mean_to_sd_all, theta_mean_to_sd_all, psi_mean_to_sd_all,
# 																									phi_mean_to_sd, theta_mean_to_sd, psi_mean_to_sd,
# 																									dist_total, omega_total, theta_total, psi_total,
# 																									phi_sd, theta_sd,
# 																									dist_mean_to_sd, omega_mean_to_sd, f1, f2)
# 
...
# used for models 9, 10, 11
# ########################################################################################################
x_train_processed <- x_train_processed %>% mutate(
																									f1 = log(dist_mean_to_sd*omega_mean_to_sd),
																									f2 = abs(atan(theta_mean_all/psi_mean_all)))
x_test_processed <- x_test_processed %>% mutate( 
																									f1 = log(dist_mean_to_sd*omega_mean_to_sd),
																									f2 = abs(atan(theta_mean_all/psi_mean_all)))
...
model_fit_RF_untuned <- randomForest(
	surface ~ ., 
	metric = "ROC", 
	data = slice(select(x_train_for_train, -series_id), 1:5000)
	)

y_hat <- predict(model_fit_RF_untuned, select(x_train_for_test, -series_id))
...
control <- trainControl(method="repeatedcv", number=20, repeats=2, search="grid", classProbs = TRUE)

metric <- "Accuracy"

mtry <- sqrt(ncol(x_train_for_train) - 1)
# mtry <- 2:5
tunegrid <- expand.grid(.mtry=mtry,.ntree=c(100, 200, 500, 1000) )
...
model_fit_rf <- train(surface ~ ., 
											data = slice(select(x_train_for_train, -series_id), 1:500), 
...
for(current_surface in surfaces$surface)
{
		...
		control <- trainControl(method="repeatedcv", number=20, repeats=2, search="grid", classProbs = TRUE, summaryFunction = twoClassSummary)
		customRF 						<- 	list(type = "Classification", library = "randomForest", loop = NULL)
		customRF$parameters <- 	data.frame(parameter = c("mtry", "ntree"), class = rep("numeric", 2), label = c("mtry", "ntree"))
		customRF$grid 			<- 	function(x, y, len = NULL, search = "grid") {}
		customRF$fit 				<- 	function(x, y, wts, param, lev, last, weights, classProbs, ...) randomForest(x, y, mtry = param$mtry, ntree=param$ntree, ...)

...
>>>>>>> 8b1e14b371c3cf14cbec8ac2890051b2a1d6e36e

# used for models 9
# ########################################################################################################
x_train_processed <- x_train_processed %>% mutate(f1 = log(dist_mean_to_sd*omega_mean_to_sd),
																									f2 = abs(atan(theta_mean_all/psi_mean_all)))
x_test_processed <- x_test_processed %>% mutate(f1 = log(dist_mean_to_sd*omega_mean_to_sd),
																									f2 = abs(atan(theta_mean_all/psi_mean_all)))
...
tic(" fit model knn")
fit <- train(surface ~ f1 + f2 ,  method = "knn", 
...
model_fit_RF_untuned <- randomForest(surface ~ ., metric = "ROC", data = select(x_train_for_train, -series_id))
...
for(current_surface in surfaces$surface)
{
		...
		x_train_for_train_ova_current <- x_train_processed_ova %>% 
			mutate(surface = ifelse(surface == current_surface, current_surface, "the_rest")) %>% 
			mutate(surface = as.factor(surface))
		...
		model_fit_current <- train(surface ~ ., 
															 data = x_train_for_train_ova_current, 
															 method=customRF, 
															 metric="ROC", 
															 tuneGrid=tunegrid, 
															 trControl=control)
...
<<<<<<< HEAD
write_csv(submission, "data/submission_08.csv")
=======
write_csv(submission, "data/submission_09.csv")
>>>>>>> 8b1e14b371c3cf14cbec8ac2890051b2a1d6e36e
```

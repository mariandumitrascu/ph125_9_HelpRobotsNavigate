...
names_to_remove_test <- findCorrelation(cor(x_test_matrix), cutoff = 0.85, names = TRUE, verbose = FALSE, exact=TRUE)
...
pre_process <- x_train_processed %>% select(-series_id) %>% preProcess(method = c("center", "scale"))
x_train_processed <- predict(pre_process, x_train_processed)
x_test_processed <- predict(pre_process, x_test_processed)

# x_train_processed <- x_train_processed %>% select(-omega_max_to_min, -theta_min, -dist_max_to_min)
# x_test_processed <- x_test_processed %>% select(-omega_max_to_min, -theta_min, -dist_max_to_min)
...
# x_train_processed <- bind_cols(select(x_train_processed, series_id, surface, f3, f2),
...
fits <- lapply(models, function(model){
  print(model)
  train(surface ~., method=model, data = select(x_train_for_train, -series_id))
...
control <- trainControl(method="repeatedcv", number=10, repeats=2, search="grid", classProbs = TRUE, sampling = "up")
...
customRF 						<- 	list(type = "Classification", library = "rf", loop = NULL)

...
x_train_processed %>%  ggplot(aes(sqrt(omega_mean_to_sd^2 + dist_mean_to_sd^2), atan(theta_mean_all/psi_mean_all), fill = surface)) +
	geom_point(aes(color = surface)) +
	geom_point(cex=6, pch=21)
...
for(current_surface in surfaces$surface)
{
		...
		# add data from pool to balance
		x_chunk <- x_train_pool %>% filter(surface == current_surface)
		x_train_for_train_ova_current <- bind_rows(x_train_for_train_ova_current, x_chunk)
...
Loop over each surface, load the model from hard-disk and perform the prediction on the test set.
...
write_csv(submission, "data/submission_06.csv")
```

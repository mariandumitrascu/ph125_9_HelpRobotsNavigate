...
pre_process <- function(a_dataframe, n_of_rows = nrow(a_dataframe)/128 ) {
	...
	group_by(series_id) %>% 
	summarize(
		# surface = first(surface), we dont have surface in test set
		mean_phi = mean(phi),
		mean_theta = mean(theta),
...
train_dataset <- pre_process(train_df)
test_dataset <- pre_process(test_df)

# rejoin train data with the labels data set
train_dataset <- train_dataset %>% left_join(y_train, by = "series_id")

# remove series_id
train_dataset <- train_dataset %>% select(-series_id)


# split data for training
test_index <- createDataPartition(y = train_dataset$surface, times = 1, p = 0.5, list = FALSE)
train_train_partition <- train_dataset[-test_index, ]
train_test_partition <- train_dataset[test_index, ]
...
train_dataset %>%  ggplot(aes(dist_t, mean_sd_dist, fill = surface)) +
	geom_point(aes(color = surface))

train_dataset %>%  ggplot(aes(mean_phi, mean_theta, fill = surface)) +
	geom_point(aes(color = surface))

train_dataset %>%  ggplot(aes(mean_phi, mean_psi, fill = surface)) +
	geom_point(aes(color = surface))

train_dataset %>%  ggplot(aes(mean_theta, mean_psi, fill = surface)) +
	geom_point(aes(color = surface))

train_dataset %>%  ggplot(aes(omega_t, mean_sd_omega, fill = surface)) +
	geom_point(aes(color = surface))

train_dataset %>%  ggplot(aes(dist_t, omega_t, fill = surface)) +
	geom_point(aes(color = surface))

train_dataset %>%  ggplot(aes(dist_t, phi_t + theta_t + psi_t, fill = surface)) +
	geom_point(aes(color = surface))

# train_dataset %>% filter(surface == "soft_tiles")
```
...
fit_model_RF_untuned <- randomForest(surface ~ ., data = train_dataset)

# predict
y_hat <- predict(fit_model_RF_untuned, select(test_dataset, -series_id))

test_dataset_for_submission <- test_dataset %>% select(series_id) %>% mutate(surface = y_hat)
write_csv(test_dataset_for_submission, "data/submission_RF_untuned0_02.csv")
...
# fit_model_rf <- train(surface~., data=train_dataset, method=customRF, metric=metric, tuneGrid=tunegrid, trControl=control)
...
test_series_id <- test_dataset %>% select(series_id)

# remove series_id from the data set in order to make the predictions
test_dataset_for_predict <- test_dataset %>% select(-series_id)

# read the saved model
fit_model_RF <- readRDS("models/fit_model_RF_one_vs_one.rds")

# predict
y_hat <- predict(fit_model_RF, test_dataset_for_predict)
